...
#ifdef HAVE_GNUTLS
	if (data->flags & SF_TRY_SECURE) {
		data->server = strdup(server_name);
...
int
iks_connect_fd (iksparser *prs, int fd, const char *server_name)
{
#ifdef USE_DEFAULT_IO
	struct stream_data *data = iks_user_data (prs);
...
#ifdef HAVE_GNUTLS
static int gnutls_recv_timeout(iksparser *prs, int timeout)
{
	struct stream_data *data = iks_user_data (prs);
	int len;
...
iks_recv (iksparser *prs, int timeout)
{
	...
	while (1) {
		...
		if (data->flags & SF_SECURE) {
			//len = gnutls_record_recv (data->sess, data->buf, NET_IO_BUF_SIZE - 1);
			len = gnutls_recv_timeout(prs, timeout);
		} else
#endif
		{
...
int
iks_start_oldtls (iksparser *prs)
{
#ifdef HAVE_GNUTLS
	struct stream_data *data = iks_user_data (prs);
